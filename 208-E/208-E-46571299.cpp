...
int main()
{
	...
	for(i=0;i<m;i++){
		cin>>v>>p;
		u=pth_anc(v,p); deep=depth[u]+p;
		if(u) cout<<((upper_bound(depths[deep].begin(),depths[deep].end(),times[u][1])-lower_bound(depths[deep].begin(),depths[deep].end(),times[u][0]))-1);
		else cout<<0;
		cout<<" ";
	}
